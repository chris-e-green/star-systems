...
class Name:CustomStringConvertible {
    ...
    init(maxLength:Int = 0, nameAlgorithm: NameAlgorithm = NameAlgorithm.frontier) {
        ...
        let u = Int(arc4random_uniform(UInt32(wordLengthCD.last!)))
        var nameLen = 1
        for index in 0..<wordLengthCD.count {
        ...
        let v = Int(arc4random_uniform(UInt32(initialLetterCD.last!)))
        var initLetter:Character = " "
        for index in 0..<initialLetterCD.count {
        ...
        var cl = initLetter
        for _ in 1..<nameLen {
            let x = followingLetterCD[cl]!
            let w = Int(arc4random_uniform(UInt32(x.last!)))
            for index in 0..<followingLetterCD[cl]!.count {
